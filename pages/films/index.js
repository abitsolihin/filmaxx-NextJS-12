...
const Genre = ({ films }) => {
    const [listFilms, setListFilm] = useState(films)
    const [query, setQuery] = useState('');

    useEffect(() => {
        const getFilms = async () => {
            const response = await axios.get(`https://filmaxx-server.vercel.app/films?t=${query}`);
        ...
        <div className="max-w-[1280px] min-h-screen mx-auto">
            <div className="container h-full w-full">
                <div className="title text-white px-4 text-2xl flex items-center py-4 justify-between">
                    ...
                    <div className="wrapper flex gap-4 px-4 py-2 bg-transparent text-white rounded-full border-2 border-red-600 border-solid">
                        <input className="focus:outline-none bg-transparent text-sm" placeholder="Cari Film Disini" type="text" value={query} onChange={handleInputChange} />
                ...
                <div className="cards h-full w-full grid grid-cols-5 gap-4 pr-4 pl-4 pb-4">
                    {listFilms.map((item, index) => {
                        ...
                        return (
                            <Link href={`/films/film/${id}`} key={index}>
